...
namespace TestConsole
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var customers = customerBD.Read();

            foreach (var customer in customers)
            {
                // Хеширование пароля пользователя с использованием bcrypt
                string hashedPassword = BCrypt.Net.BCrypt.HashPassword(customer.Password); // хеширование пароля

                // Обновление хешированного пароля в базе данных
                customerBD.UpdateCustomerPassword(customer.Id, hashedPassword);
            }
